...
## 2026-06-29 绑定任务栈溢出根因

- `device_bind` 任务原栈大小 6144 字节，但绑定请求里放了 4096 字节响应数组，并叠加 HTTP client/cJSON 调用栈，导致启动后 WiFi 获取 IP 触发绑定任务时立即栈溢出重启。
- 后端绑定接口本身可用；设备重启不是 BLE、SD、图片或 WiFi 配置问题。
- 该类后台任务不要在任务栈上放 KB 级响应缓冲，应使用 heap/PSRAM，并限制失败日志只输出响应前缀。
